...
let _teamSso = {};      // teamId -> {issuer, clientId, clientSecret, domains, enabled, enforced, updatedAt, updatedBy}

const MAX_SSO_DOMAINS = 10;
const _oidc = require('./oidc');
...
function initTeams(dataDir) {
  ...
  _ssoFile = path.join(dataDir, 'team-sso.json');
  ...
  const sso = loadJson(_ssoFile, {});
  _teamSso = (sso && typeof sso === 'object' && !Array.isArray(sso)) ? sso : {};
...
function _saveTeamSso() { saveJson(_ssoFile, _teamSso); }
...
function _isPaid(user) {
  // Deliberately the TEAM tier, not merely any paying tier: Pro is a paid
  // individual plan and must not unlock the shared library.
  return !!(user && plans.canUseTeams(user.plan));
}

/**
 * Does this team still contain at least one paying member?
 *
 * The paid gate used to exist ONLY at create/join time (_isPaid in createTeam
 * and acceptInvite), which was fine while upgrades were manual and permanent.
 * With a real cancellation path it is not: setUserPlan(free) flips a field that
 * gates nothing once the team exists, so a month of payment bought the team
 * forever. This is the check that makes lapsing mean something.
 * @param {string} teamId
 * @returns {boolean}
 */
function teamHasPaidMember(teamId) {
  _ensureInit();
  for (const uid of Object.keys(_members)) {
    const m = _members[uid];
    if (!m || m.teamId !== teamId || m.suspended) continue;
    if (_isPaid(_userById(uid))) return true;
  }
  return false;
}
...
// Team-scoped state (who the IdP is, which email domains it may sign in) lives
// here alongside the rest of a team's data. The protocol mechanics (PKCE,
// discovery validation, claims checking) live in lib/oidc.js; the actual HTTP
// calls to the IdP and the request handlers live in lib/sso.js and server.js.
// This module's job is exactly what it already does for everything else: own
// the persisted shape and validate what goes into it.

/**
 * A team's SSO configuration, or null. Includes the plaintext client secret
 * -- this is an internal accessor for lib/sso.js's login flow, never for a
 * client response. An API route wanting to show this to a browser must mask
 * the secret itself (see setTeamSso's own return value, which already does).
 * @param {string} teamId
 * @returns {object|null}
 */
function getTeamSso(teamId) {
  _ensureInit();
  return _teamSso[teamId] || null;
}

/**
 * Find the (enabled) SSO config claiming a given email domain, or null.
 * Domains are globally unique across teams (enforced in setTeamSso), so this
 * can only ever match one config.
 * @param {string} domain lowercased domain, e.g. from oidc.emailDomain()
 * @returns {object|null}
 */
function findTeamSsoByDomain(domain) {
  _ensureInit();
  if (!domain) return null;
  for (const teamId of Object.keys(_teamSso)) {
    const cfg = _teamSso[teamId];
    if (cfg && cfg.enabled !== false && Array.isArray(cfg.domains) && cfg.domains.includes(domain)) {
      return cfg;
    }
  }
  return null;
...
/** A user-safe view of a team's SSO config: same shape, secret replaced with a boolean. */
function _maskTeamSso(cfg) {
  if (!cfg) return null;
  return {
    issuer: cfg.issuer,
...
/**
 * Save (create or update) a team's SSO configuration, after validating every
 * field server-side -- this is the only place bad IdP config can be kept out,
 * since nothing stops a browser from calling the API directly.
 *
 * `clientSecret` is write-only: an empty string in `input` means "keep
 * whatever is already stored" (so a re-save that only changes, say, the
 * domain list does not force re-entering the secret) rather than "clear it".
 *
 * @param {string} teamId
 * @param {{issuer:string, clientId:string, clientSecret:string,
 *   domains:(string[]|string), enabled?:boolean, enforced?:boolean}} input
 * @param {string} actingUserId whoever saved this, for the audit trail
 * @returns {object} the masked config (see _maskTeamSso)
 * @throws {Error} with .userMessage on any validation failure
 */
function setTeamSso(teamId, input, actingUserId) {
  _ensureInit();
  const team = _teams[teamId];
  if (!team) throw _err('No such team.');

  const issuer = String((input && input.issuer) || '').trim();
  if (!issuer || issuer.length > 300) throw _err('Issuer URL is required (max 300 characters).');
  let issuerUrl;
  try { issuerUrl = new URL(issuer); } catch { throw _err('Issuer must be a valid URL.'); }
  if (issuerUrl.protocol !== 'https:') throw _err('Issuer must be an https:// URL.');
  if (issuerUrl.search || issuerUrl.hash) throw _err('Issuer must not contain a query string or fragment.');
  if (/^\d+\.\d+\.\d+\.\d+$/.test(issuerUrl.hostname) || issuerUrl.hostname.includes(':')) {
    throw _err('Issuer must use a hostname, not a bare IP address.');
  }

  const clientId = String((input && input.clientId) || '').trim();
  if (!clientId || clientId.length > 200) throw _err('Client ID is required (max 200 characters).');

  const prev = _teamSso[teamId];
  // Blank in the request keeps the previously stored secret -- see the
  // function doc. After that fallback it must still be non-empty: this
  // endpoint can update or replace a secret but never silently erase it.
  const clientSecret = String((input && input.clientSecret) || '') || (prev && prev.clientSecret) || '';
  if (!clientSecret) throw _err('Client secret is required.');
  if (clientSecret.length > 500) throw _err('Client secret is too long.');

  const rawDomains = Array.isArray(input && input.domains)
    ? input.domains
    : String((input && input.domains) || '').split(',');
  const domains = [...new Set(rawDomains.map((d) => String(d).trim().toLowerCase()).filter(Boolean))];
  if (!domains.length || domains.length > MAX_SSO_DOMAINS) {
    throw _err('Claim between 1 and ' + MAX_SSO_DOMAINS + ' email domains.');
  }
  for (const d of domains) {
    if (!_oidc.isValidDomainName(d)) throw _err('"' + d + '" is not a valid domain name.');
    if (_oidc.isPublicEmailDomain(d)) {
      throw _err('"' + d + '" is a public email provider and cannot be claimed for SSO.');
    }
    for (const otherId of Object.keys(_teamSso)) {
      const other = _teamSso[otherId];
      if (otherId !== teamId && other && Array.isArray(other.domains) && other.domains.includes(d)) {
        throw _err('"' + d + '" is already claimed by another workspace.');
      }
    }
  }
  ...
  _teamSso[teamId] = {
    ...
    enforced: !!(input && input.enforced),
    updatedAt: new Date().toISOString(),
    updatedBy: actingUserId,
  };
  _saveTeamSso();
  return _maskTeamSso(_teamSso[teamId]);
}

/**
 * Drop a team's SSO configuration.
 *
 * Members who were JIT-provisioned via SSO (no password) are deliberately
 * left as-is -- their `user.sso` field goes inert (no config → no route in),
 * not deleted, so "Forgot password" still recovers their account rather than
 * losing it outright.
 * @param {string} teamId
 * @returns {boolean} true if a config existed and was removed
 */
function deleteTeamSso(teamId) {
  _ensureInit();
  if (!_teamSso[teamId]) return false;
  delete _teamSso[teamId];
  _saveTeamSso();
  return true;
}

/**
 * Add a JIT-provisioned SSO user to a team, exactly like acceptInvite's own
 * member-creation step -- same shape, same MAX_TEAM_MEMBERS ceiling -- just
 * without an invite token to consume, because SSO is the invite.
 * @param {string} userId a user just created for this sign-in
 * @param {string} teamId
 * @throws {Error} with .userMessage if the team is full or the user is
 *   already on a (different) team
 */
function addMemberForSso(userId, teamId) {
  _ensureInit();
  if (getTeamIdFor(userId)) throw _err('This account is already on a team.');
  if (_teamMemberCount(teamId) >= MAX_TEAM_MEMBERS) {
    throw _err('This team already has the maximum of ' + MAX_TEAM_MEMBERS + ' members.');
  }
  _members[userId] = { teamId, role: 'member', joinedAt: new Date().toISOString(), suspended: false };
  _saveMembers();
}
...
function createInvite(inviterUserId, email) {
  ...
  // Inviting is growth, and growth is the thing being paid for. Without this a
  // cancelled owner could keep adding people to a team nobody pays for.
  if (!_isPaid(_userById(inviterUserId))) {
    throw _err('Inviting members needs an active paid account. Renew your subscription, then invite again.');
  }
 ...
 */
async function acceptInvite(token, opts) {
  _ensureInit();
  opts = opts || {};
  _sweepInvites();
  ...
  if (sessionUser && String(sessionUser.email).toLowerCase() === invitedEmail) {
    ...
    if (existing) {
      ...
      const verified = await auth.verifyPassword(invitedEmail, pw);
...
/**
 * Leave the team voluntarily. This is deliberately NOT removeMember(self, self):
 * removeMember refuses a plain member (not canManageMembers) AND refuses anyone
 * acting on themselves, so it can never express a leave. server.js's account
 * deletion used to call it that way, the throw was swallowed by a bare catch,
 * and the member row survived the account -- a ghost row that still counted
 * against MAX_TEAM_MEMBERS and rendered as a raw hex id with no email.
 *
 * The owner may not leave: their id is the team's dataRootId, and a team with
 * no owner is exactly the stranded state claimOwnership() exists to repair.
 * They must transferOwnership() first -- an explicit handover, not an implicit one.
 * @param {string} userId
 * @returns {{ok:true, teamId:string}}
 * @throws {Error} with .userMessage
 */
function leaveTeam(userId) {
  _ensureInit();
  const m = _members[userId];
  if (!m) throw _err('You are not on a team.');
  const teamId = m.teamId;

  if (m.role === 'owner') {
    // A team of one is dissolved rather than defended: there is nobody to hand
    // it to, and refusing would trap the last person in a team they cannot
    // leave. With other members present the owner must transfer first --
    // otherwise the team loses its owner and every remaining member has to go
    // through claimOwnership() to get management rights back.
    if (_teamMemberCount(teamId) > 1) {
      throw _err('You own this team, so you cannot leave it. Transfer ownership to another member first, then leave.');
    }
    delete _members[userId];
    delete _teams[teamId];
    // Pending invites to a team that no longer exists would 500 on acceptance.
    for (const token of Object.keys(_invites)) {
      if (_invites[token] && _invites[token].teamId === teamId) delete _invites[token];
    }
    _saveMembers();
    _saveTeams();
    _saveInvites();
    return { ok: true, teamId, dissolved: true };
  }

  delete _members[userId];
  _saveMembers();
  return { ok: true, teamId, dissolved: false };
}

/**
 * How long an owner must be absent before a member may take the team over.
 * 90 days is deliberately long: a claim is irreversible for the old owner
 * (only an owner can transfer, and they will no longer be one), so the bar
 * should be "clearly gone", not "on a long holiday".
 */
const OWNER_DORMANT_DAYS = 90;

/** ms since an ISO timestamp, or null when it is missing/unparseable. */
...
/**
 * Whether this user could take ownership of their team right now, and why not
 * if they cannot. Drives the UI so the answer shown to the user and the answer
 * enforced by claimOwnership() come from ONE piece of logic rather than two
 * that can drift apart.
 *
 * Threat model, and why this is a smaller lever than it first appears: every
 * member of a team already reads and writes the same shared data root
 * (accountUid()), so claiming ownership grants MANAGEMENT rights -- invite,
 * suspend, remove, transfer -- not any new access to captures or guides. The
 * realistic abuse is therefore "a member seizes control of a team whose owner
 * is merely quiet", which the dormancy window and the admin-first rule below
 * are what guard against.
 *
 * @param {string} userId
 * @returns {{claimable:boolean, reason:string, ownerEmail:string|null,
 *            ownerLastActiveAt:string|null, daysSinceOwnerActive:number|null,
 *            dormantAfterDays:number}}
 */
function getRecoveryState(userId) {
  _ensureInit();
  const out = {
    claimable: false,
    reason: 'not-on-a-team',
    ownerEmail: null,
    ownerLastActiveAt: null,
    daysSinceOwnerActive: null,
    dormantAfterDays: OWNER_DORMANT_DAYS,
  };
  const me = _members[userId];
  if (!me) return out;
  const team = _teams[me.teamId];
  if (!team) return out;

  if (me.role === 'owner') { out.reason = 'you-are-the-owner'; return out; }
  // A suspended member seizing the team would let them un-suspend themselves,
  // which turns a moderation action into a takeover. Never claimable.
  if (me.suspended) { out.reason = 'suspended'; return out; }

  const owner = _userById(team.ownerId);
  if (!owner) {
    // The owner's account is gone entirely -- nobody is coming back. No wait.
    out.reason = 'orphaned';
    out.claimable = true;
  } else {
    out.ownerEmail = owner.email || null;
    // lastLoginAt is null until the owner's first login after that field
    // existed, so fall back to createdAt rather than treating unknown as
    // "dormant forever" and handing the team away on day one.
    const lastActive = owner.lastLoginAt || owner.createdAt || null;
    out.ownerLastActiveAt = lastActive;
    const since = _msSince(lastActive);
    if (since === null) { out.reason = 'owner-activity-unknown'; return out; }
    out.daysSinceOwnerActive = Math.floor(since / 86400000);
    if (out.daysSinceOwnerActive < OWNER_DORMANT_DAYS) {
      out.reason = 'owner-active';
      return out;
    }
    out.reason = 'dormant';
    out.claimable = true;
  ...
  // Admin-first: while any active admin remains, recovery is their job. This
  // stops an ordinary member from taking a team over the heads of the people
  // the owner actually delegated to. If no admin is left, any active member
  // may claim -- otherwise a team whose only admin also vanished stays stuck,
  // which is the very failure this function exists to fix.
  const admins = Object.keys(_members).filter((uid) => {
    const m = _members[uid];
    return m && m.teamId === me.teamId && m.role === 'admin' && !m.suspended;
  });
  if (admins.length && me.role !== 'admin') {
    out.claimable = false;
    out.reason = 'admins-must-claim';
  }
  return out;
}

/**
 * Take ownership of a team whose owner is gone or long dormant. Mirrors
 * transferOwnership's data rules exactly: team.dataRootId is NEVER touched
 * (it would orphan every capture and guide the team owns), and a still-extant
 * old owner is demoted to admin rather than removed, so someone returning
 * after a long absence finds themselves demoted, not locked out.
 * @param {string} userId the member taking over
 * @returns {{ok:true, teamId:string, previousOwnerRemoved:boolean}}
 * @throws {Error} with .userMessage
 */
function claimOwnership(userId) {
  _ensureInit();
  const state = getRecoveryState(userId);
  if (!state.claimable) {
    const messages = {
      'not-on-a-team': 'You are not on a team.',
      'you-are-the-owner': 'You already own this team.',
      suspended: 'Your membership is suspended, so you cannot take over this team.',
      'owner-active': 'The owner of this team is still active, so it cannot be taken over. Ask them to transfer ownership.',
      'owner-activity-unknown': 'This team cannot be taken over yet — the owner has no recorded activity to measure.',
      'admins-must-claim': 'A team admin has to take this team over. Ask one of them to do it.',
    };
    throw _err(messages[state.reason] || 'This team cannot be taken over right now.');
  }
  ...
  team.ownerId = userId;                    // dataRootId deliberately untouched
  me.role = 'owner';
  me.suspended = false;

  let previousOwnerRemoved = false;
  if (_members[oldOwnerId]) {
    if (_userById(oldOwnerId)) {
      _members[oldOwnerId].role = 'admin';  // demoted, not evicted
    } else {
      delete _members[oldOwnerId];          // stale row for a deleted account
      previousOwnerRemoved = true;
    }
  }

  _saveTeams();
  _saveMembers();
  return { ok: true, teamId, previousOwnerRemoved };
}
