 ...
 *   stripePriceMonthly     price_           Team, recurring monthly
 *   stripePriceAnnual      price_           Team, recurring yearly
 *   stripeProPriceMonthly  price_           Pro, recurring monthly (optional)
 *   stripeProPriceAnnual   price_           Pro, recurring yearly  (optional)
 *
 * Price IDs are not secrets -- they are public identifiers that appear in
 * checkout URLs -- so unlike the key and webhook secret they live in
 * config.json rather than the service environment.
 *
...
/**
 * Configure from data/config.json, with the ENVIRONMENT taking precedence for
 * the two secrets.
 *
 * Why the env override exists: backup-hiccup.ps1 mirrors the whole tree,
 * including data/, and keeps 30 dated snapshots. A secret key pasted into
 * config.json therefore becomes ~31 plaintext copies on the backup drive, and
 * once it is in those snapshots the only real remediation is rotating the key
 * at Stripe. Putting the secrets in the service environment (NSSM
 * AppEnvironmentExtra) keeps them out of the tree the backup copies.
 *
 * The price ids are NOT secret and stay in config.json quite happily.
 * @param {object} config the loaded data/config.json
 */
function initStripe(config) {
  const c = (config && typeof config === 'object') ? config : {};
  const env = process.env || {};
  _cfg = Object.assign({}, c, {
    stripeSecretKey: env.STRIPE_SECRET_KEY || c.stripeSecretKey || '',
    stripeWebhookSecret: env.STRIPE_WEBHOOK_SECRET || c.stripeWebhookSecret || '',
  });
}

/** Where each secret came from, for the boot log and /admin/status. Never the value. */
function configSource() {
  const env = process.env || {};
  return {
    secretKey: env.STRIPE_SECRET_KEY ? 'env' : (_cfg.stripeSecretKey ? 'config.json' : 'unset'),
    webhookSecret: env.STRIPE_WEBHOOK_SECRET ? 'env' : (_cfg.stripeWebhookSecret ? 'config.json' : 'unset'),
  };
}
...
/**
 * Is the Pro (individual) tier sellable? Separate from isConfigured() on
 * purpose: an operator can run Team-only, and a missing Pro price should hide
 * that tier rather than break checkout for everyone.
 */
function hasProPricing() {
  return !!(isConfigured() && _str(_cfg.stripeProPriceMonthly) && _str(_cfg.stripeProPriceAnnual));
}

/**
 * True when the configured key is a live (real money) key rather than a test one.
 *
 * Both prefixes count. `sk_live_` is a full secret key; `rk_live_` is a
 * RESTRICTED key — narrower permissions, same live account, same real money.
 * Matching only `sk_` would report a restricted live key as "TEST", which is
 * exactly backwards for the one thing this function is used for: warning a
 * human before they touch production.
 */
function isLiveMode() {
  const key = _str(_cfg.stripeSecretKey);
  return key.indexOf('sk_live_') === 0 || key.indexOf('rk_live_') === 0;
}

/**
 * The price id for a (tier, interval) pair, or null.
 *
 * The client supplies neither -- it names a tier and an interval, and this
 * maps them to configured ids. A client that could name a price could name
 * its own price.
 * @param {'pro'|'team'} tier
...
function priceIdFor(tier, interval) {
  ...
  if (tier === 'team') {
    return _str(monthly ? _cfg.stripePriceMonthly : _cfg.stripePriceAnnual) || null;
  }
  if (tier === 'pro') {
    return _str(monthly ? _cfg.stripeProPriceMonthly : _cfg.stripeProPriceAnnual) || null;
  }
  return null;
}
...
async function createCheckoutSession(opts) {
  const o = opts || {};
  const price = priceIdFor(o.tier, o.interval);
  if (!price) {
    throw _err('Choose a plan and a billing period.',
      'stripe: no price for tier=' + o.tier + ' interval=' + o.interval);
  }
  ...
  const body = {
    ...
    // hiccup_plan rides along so the webhook knows WHICH tier was bought.
    // Reading it back beats expanding line items or making a second call,
    // and it is set by us, never by the browser.
    subscription_data: { metadata: { hiccup_user_id: o.userId, hiccup_plan: o.tier } },
    metadata: { hiccup_user_id: o.userId, hiccup_plan: o.tier },
...
module.exports = {
  ...
  verifyWebhook,
  configSource,
  // Underscored because it is not part of the app's surface: the only caller
  // is bin/stripe-prices.js, the one-shot price-setup tool. Nothing that
  // serves a request should be reaching for a raw API call.
  _api,
};
