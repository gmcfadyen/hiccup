...
/**
 * Strip a trailing run of any of `chars`.
 *
 * The regex form (/[),.;]+$/) has no left anchor, so the engine retries the run
 * from every offset — quadratic on a long token from an uploaded log.
 *
 * @param {string} s
...
function stripTrailing(s, chars) {
  let end = s.length;
  while (end > 0 && chars.indexOf(s.charAt(end - 1)) !== -1) end--;
...
function parseAddr(token) {
  let t = stripTrailing(String(token == null ? '' : token), '),.;');
  const bm = /^\[([^\]]+)\](?::(\d+))?$/.exec(t);
...
function topVia(headerLines) {
  for (let k = 1; k < headerLines.length; k++) {
    ...
    // Any SIP/<digits>.<digits>, to match the relaxed REQ_LINE/STATUS_LINE.
    // Hardcoding 2.0 here meant a SIP/7.0 message became recognisable as a
    // message but its Via still never parsed, so src/dst silently fell back
    // to unknown-a/unknown-b and transport defaulted to udp.
    const vm = /SIP\s*\/\s*\d{1,3}\.\d{1,3}\s*\/\s*([A-Za-z]+)\s+([^\s;,]+)/.exec(firstVal);
    if (!vm) return null;
