...
function buildChatBody(model, system, messages, keepAlive, tools, numPredict) {
  const msgs = [];
  if (system) msgs.push({ role: 'system', content: String(system) });
  for (const m of messages) {
    // The tool-loop conversation carries two shapes a plain chat never sends:
    // an assistant turn WITH its tool_calls echoed back (the model needs to
    // see what it asked for), and role:'tool' results. Anything else is
    // coerced to user/assistant exactly as before, so existing callers see
    // no change.
    const role = (m && (m.role === 'assistant' || m.role === 'tool')) ? m.role : 'user';
    const out = { role, content: String((m && m.content) || '') };
    if (role === 'assistant' && Array.isArray(m.tool_calls) && m.tool_calls.length) {
      out.tool_calls = m.tool_calls;
    }
    if (role === 'tool' && m.tool_name) out.tool_name = String(m.tool_name);
    msgs.push(out);
  }
  const body = {
    model,
    ...
    options: {
      ...
      // 700 is right for a one-shot answer; the agent's FINAL call synthesises
      // several tool results and asks for more headroom. Capped so a caller
      // can never turn one request into an unbounded generation.
      num_predict: Math.min(Number(numPredict) > 0 ? Number(numPredict) : 700, 1200),
    },
  };
    ...
    const req = lib.request({
      ...
      res.on('end', () => {
        ...
        if (res.statusCode >= 400) {
          ...
          e.statusCode = res.statusCode;
          // A model with no tool template rejects the whole request. The
          // agent path catches this code and falls back to a plain one-shot
          // answer, so tool support degrading never takes chat down.
          if (/does not support tools/i.test(text)) e.code = 'no-tools';
          return reject(e);
        }
        let j;
        ...
        const content = (j && j.message && typeof j.message.content === 'string')
          ? j.message.content : '';
        const toolCalls = (j && j.message && Array.isArray(j.message.tool_calls))
          ? j.message.tool_calls : [];
        resolve({ text: content, model: (j && j.model) || body.model, toolCalls });
      });
    });
...
async function runJob(item) {
  ...
  const body = buildChatBody(pick.model, item.system, item.messages, pick.keepAlive, item.tools, item.numPredict);
  try {
 ...
 * @param {Array<{role: 'user'|'assistant'|'tool', content: string}>} opts.messages
 *   - assistant entries may carry `tool_calls`, tool entries `tool_name`
 *   (the agent loop's conversation shape; plain chat never needs either).
 * @param {Array<object>} [opts.tools] - Ollama tool definitions. When the
 *   model answers with tool calls, the result's `toolCalls` is non-empty and
 *   `text` may be empty. A model with no tool template rejects with
 *   err.code='no-tools' — callers fall back to a plain ask.
 * @param {number} [opts.priority] - queue priority (paid tiers jump ahead of
 *   priority-0 jobs already queued; stable within a tier).
 * @returns {Promise<{text: string, model: string, toolCalls: Array<object>}>}
 */
function askLlm(opts) {
  return new Promise((resolve, reject) => {
    ...
    // Priority insert, not a plain push. A paid tier is sold on being served
    // first when the queue is busy, so that has to be true in the queue rather
    // than only on the pricing page.
    //
    // Insertion is STABLE within a priority: a job only jumps ahead of jobs
    // with a strictly lower priority, so free requests keep their order among
    // themselves and are never starved by a steady trickle of paid ones -- they
    // wait behind the paid jobs already queued, not behind every future one.
    const priority = Number(o.priority) > 0 ? Number(o.priority) : 0;
    const tools = (Array.isArray(o.tools) && o.tools.length) ? o.tools : null;
    const job = { system: o.system || null, messages, tools,
      numPredict: o.numPredict, resolve, reject, priority };
    let at = queue.length;
    if (priority > 0) {
      while (at > 0 && queue[at - 1].priority < priority) at--;
    }
...
module.exports = {
  initLlm, getLlmStatus, askLlm,
  // exported for tests only — the request shape is a contract worth pinning
  _buildChatBody: buildChatBody,
};
