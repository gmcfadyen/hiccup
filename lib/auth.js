...
/**
 * scrypt on libuv's threadpool instead of the event loop.
 *
 ...
 * the lot -- for that whole time. A burst of sign-in attempts was therefore a
 * self-inflicted denial of service against real users, which is what made the
 * missing rate limit worth more than it first looked.
 *
 * The threadpool defaults to 4 threads, so concurrent hashes queue there rather
 * than stalling the loop. Raise UV_THREADPOOL_SIZE if sign-in concurrency ever
...
let _dummyHashPromise = null;  // memoised promise for the unknown-email timing path

/**
 * The hash burned for unknown emails so a miss costs the same as a hit.
 * The PROMISE is memoised, not the value, so concurrent first-time callers
 * share one computation instead of each starting their own.
 */
function _dummyHash() {
  if (!_dummyHashPromise) _dummyHashPromise = _hashPassword('hiccup-dummy-password-for-timing');
...
/** scrypt-hash a password into 'scrypt:N:r:p:saltHex:hashHex'. Async -- see scryptAsync. */
...
/** Timing-safe check of a password against a stored 'scrypt:...' string. Async. */
async function _checkPassword(password, stored) {
  ...
  let key;
  try {
    key = await scryptAsync(String(password), Buffer.from(parts[4], 'hex'),
      SCRYPT_KEYLEN, { N, r, p });
  } catch {
    return false;
  }
...
function _publicUser(u) {
  return {
    ...
    // 'free' for every record created before this field existed — an absent
    // plan must read as the least-privileged one, never as an accidental grant.
...
function initAuth(dataDir, config) {
  ...
  // The dummy hash is built lazily (see _dummyHash): initAuth is synchronous
  // and is called at module scope by server.js, so it cannot await one here.
 ...
 */
async function createUser({ email, password, name, googleSub, sso } = {}) {
  _ensureInit();
  ...
  if (existing) {
    ...
    // SSO never reaches this branch in practice -- lib/sso.js's account-
    // linking policy looks an existing email up itself before ever calling
    // createUser, and links or refuses rather than creating a duplicate.
    // This throw is the backstop if that invariant is ever violated.
    throw _err('An account with that email already exists.');
  }

  // Both googleSub and sso mean "an identity provider already vouched for
  // this person" -- no password is required, or none was supplied.
  if (!googleSub && !sso) {
  ...
  const user = {
    ...
    passwordHash: (typeof password === 'string' && password.length >= 8)
      ? await _hashPassword(password) : null,
    googleSub: googleSub || null,
    // {teamId, sub, iss} for a user JIT-provisioned or linked via an
    // enterprise IdP. Never sent to a client -- see findRawUserById's
    // comment on why a raw accessor exists at all.
    sso: sso ? { teamId: String(sso.teamId), sub: String(sso.sub), iss: String(sso.iss) } : null,
...
async function verifyPassword(email, password) {
  ...
  if (!user || !user.passwordHash) {
    await _checkPassword(String(password || ''), await _dummyHash()); // no existence leak
    return null;
  }
  if (!(await _checkPassword(String(password || ''), user.passwordHash))) return null;
...
/**
 * Raw (non-public) user record by email — has passwordHash/googleSub/sso.
 *
 * This exists ONLY for lib/sso.js's account-linking policy, which has to ask
 * questions _publicUser() deliberately hides an answer to: does this email
 * already have a password (so linking would silently disable it?), is it
 * already linked to a DIFFERENT team's IdP? Never return this from an API
 * route — a caller that wants the safe view should use findUserByEmail.
 * @param {string} email
 * @returns {object|null} the raw record, or null
 */
function findRawUserByEmail(email) {
  _ensureInit();
  const em = String(email || '').trim().toLowerCase();
  return _users.find(u => u.email === em) || null;
}

/**
 * Raw (non-public) user record by id. See findRawUserByEmail's warning —
...
/**
 * The user previously JIT-provisioned or linked for this exact (team, sub)
 * pair — i.e. someone who has signed in via this team's SSO before.
 * @param {string} teamId
 * @param {string} sub the IdP's stable subject identifier
 * @returns {object|null} raw record (see findRawUserByEmail's warning), or null
 */
function findUserBySso(teamId, sub) {
  _ensureInit();
  return _users.find(u => u.sso && u.sso.teamId === teamId && u.sso.sub === sub) || null;
}

/**
 * Attach an SSO identity to an EXISTING user record — the "invitation-
 * anchored linking" case: someone already has a hiccup account (with a
 * password, most likely) and is turning SSO on for the first time. Deliberately
 * separate from createUser's own sso handling, which only ever fires for a
 * brand-new record.
 * @param {string} userId
 * @param {{teamId:string, sub:string, iss:string}} sso
 * @returns {object|null} public user, or null if no such user
 */
function linkUserSso(userId, sso) {
  _ensureInit();
  const user = _users.find(u => u.id === userId);
  if (!user) return null;
  user.sso = { teamId: String(sso.teamId), sub: String(sso.sub), iss: String(sso.iss) };
  _saveUsers();
  return _publicUser(user);
}
...
/**
 * Set a user's subscription plan. Granted by the Stripe webhook on a
 * successful payment, revoked by it on cancellation/refund/dispute, and
 * settable by hand from the site-admin user-management panel.
 *
 * The legacy id 'paid' is still accepted and stored as 'team': it is what a
 * paid subscription meant before the tiers existed, so honouring it keeps
 * old records and any older caller working rather than throwing at them.
 * @param {string} userId
 * @param {'free'|'pro'|'team'|'paid'} plan
 * @returns {object|null} the updated public user, or null if no such user
 */
function setUserPlan(userId, plan) {
  _ensureInit();
  if (plans.all().indexOf(plan) === -1 && plan !== 'paid') {
    throw _err('plan must be one of: ' + plans.all().join(', ') + '.');
  }
  const user = _users.find((u) => u && u.id === userId);
  if (!user) return null;
  user.plan = plans.normalise(plan);
  _saveUsers();
  return _publicUser(user);
}

/**
 * Record the Stripe ids for a user, so a later webhook (a cancellation, a
 * failed renewal) can be traced back to the account without asking Stripe.
 *
 * These are deliberately NOT in _publicUser(): a customer id is operator
 * data. Nothing in the browser needs it, and anything it did enable there
 * would be better done server-side anyway.
 * @param {string} userId
 * @param {{customerId?:string, subscriptionId?:string|null}} ids
 * @returns {object|null} public user, or null when there is no such user
 */
function setUserBilling(userId, ids) {
  _ensureInit();
  const user = _users.find((u) => u && u.id === userId);
  if (!user) return null;
  const o = ids || {};
  if (typeof o.customerId === 'string' && o.customerId) user.stripeCustomerId = o.customerId;
  if (o.subscriptionId !== undefined) {
    user.stripeSubscriptionId = o.subscriptionId ? String(o.subscriptionId) : null;
  }
  _saveUsers();
  return _publicUser(user);
}
...
/**
 * The user a Stripe customer id belongs to, or null.
 * Used by webhook handlers for events that carry a customer but no
 * client_reference_id (a cancellation, for instance).
...
function findUserByStripeCustomer(customerId) {
  ...
  if (!id) return null;
  const u = _users.find((x) => x && x.stripeCustomerId === id);
  return u ? _publicUser(u) : null;
}

/**
 * The Stripe subscription id recorded for a user, or null. Lets a webhook
 * ignore a stale event about a subscription the account has since replaced.
 * @param {string} userId
 * @returns {string|null}
 */
/**
 * The Stripe customer id recorded for a user, or null. Passing this back to
 * Checkout keeps one person on ONE Stripe customer instead of minting a fresh
 * one per attempt.
 * @param {string} userId
 * @returns {string|null}
 */
function getUserStripeCustomer(userId) {
  _ensureInit();
  const u = _users.find((x) => x && x.id === userId);
  return (u && u.stripeCustomerId) || null;
}

function getUserSubscriptionId(userId) {
  _ensureInit();
  const u = _users.find((x) => x && x.id === userId);
  return (u && u.stripeSubscriptionId) || null;
}
