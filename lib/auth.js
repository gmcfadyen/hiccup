...
/**
 * Set a user's subscription plan. There is no payment gateway wired up yet
 * (see ARCHITECTURE.md) — this is the one place 'paid' status is granted,
 * called from the site-admin user-management panel after a Buy Me a Coffee
 * payment is seen manually.
 * @param {string} userId
...
function setUserPlan(userId, plan) {
  _ensureInit();
  if (plan !== 'free' && plan !== 'paid') throw _err('plan must be "free" or "paid".');
