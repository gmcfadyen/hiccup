...
/**
 * Clean a guide-link URL → trimmed string or null.
 *
 * http(s) only. This is stored verbatim and later rendered as an <a href> on
 * the client (kb.js), so a scheme check here is load-bearing, not cosmetic:
 * without it, a `javascript:`/`data:` URL typed into the link field would be
 * stored and then handed straight to `href`, which is stored XSS the moment
 * anyone — including the person who added it — clicks their own link.
 * @param {*} v
 * @returns {string|null}
 */
...
/**
 * Strip a trailing run of colons and whitespace.
 *
 * The regex (/[:\s]+$/) has no left anchor, so the engine retries the run from
 * every offset in the string — quadratic on a long heading from a fetched page.
 *
 * @param {string} s
 * @returns {string}
 */
function _stripTrailingColonSpace(s) {
  let end = s.length;
  while (end > 0) {
...
async function _extractPdf(buf, warnings) {
  ...
  // Two incompatible majors, and BOTH are supported deliberately.
  //   v1: `module.exports = function (buffer, opts) -> {text, numpages}`
  //   v2: a rewrite exporting a namespace object whose `PDFParse` class does
  //       the work. It ships a CJS build, so `require()` SUCCEEDS and only the
  //       call fails — which is why this looked like "pdf-parse not installed"
  //       long after it was installed. package.json asks for ^2.4.5, so v2 is
  //       what a fresh `npm install` actually produces; the v1 branch stays so
  //       an existing v1 install keeps working.
  const v1 = typeof mod === 'function' ? mod
    : (mod && typeof mod.default === 'function') ? mod.default
      : null;
  const PDFParse = (mod && typeof mod.PDFParse === 'function') ? mod.PDFParse : null;
  if (!v1 && !PDFParse) {
    throw _err(PDF_DEP_MSG, 'kb: pdf-parse exported neither a v1 function nor a v2 PDFParse class');
  }
  ...
  if (PDFParse) {
    ...
    try {
      ...
      if (pages.length) {
        // Join on \f so _splitPages() recovers real page numbers, exactly as
        // it does for v1's _pdfPageRender output. Positional, so an empty
        // (image-only) page still consumes its slot and later pages keep
        // their true numbers. The trailing \f yields a blank final part,
        // which _splitPages skips.
        text = pages.map((p) => String((p && p.text) || '')).join('\f') + '\f';
        pageCount = Number.isFinite(r.total) ? r.total : pages.length;
      } else {
      ...
      throw _err('That PDF could not be read — it may be encrypted, damaged or image-only.',
        'kb: pdf-parse v2 failed: ' + (e && e.message));
    } finally {
      // v2 holds a worker open; leaking one per upload would be a slow bleed.
    ...
    let data = null;
    try {
      data = await v1(buf, { pagerender: _pdfPageRender });
    } catch (e1) {
      try {
        data = await v1(buf);                          // default renderer
        warnings.push('page numbers unavailable for this PDF');
      } catch (e2) {
        throw _err('That PDF could not be read — it may be encrypted, damaged or image-only.',
    ...
    pageCount = data && Number.isFinite(data.numpages) ? data.numpages : null;
  }
...
function _allCapsHeading(line) {
  ...
  return _stripTrailingColonSpace(t).slice(0, MAX_HEADING_LEN);
}
...
/**
 * Register a link to a vendor's own hosted guide, instead of ingesting a
 * copy of it.
 *
 * hiccup stores nothing about the document beyond what is typed here — no
 * fetch, no extracted text, no search index entry. That is deliberate: this
 * exists in place of the originally-planned "pre-ingest vendor SBC config
 * guides" feature, which stalled on exactly the question a stored, indexed
 * copy would raise — hiccup does not hold a redistribution licence for any
 * vendor's documentation. A link has no such question because hiccup never
 * copies anything; it is a labelled bookmark back to the vendor's own site,
 * same directory layout as an uploaded guide (so listing/deleting reuse the
 * same code) but with `chunks: 0` and no `chunks.json`, so it never
 * contributes to search or citations.
 *
 * @param {{userId: string, url: string, title?: string, vendor?: string, product?: string}} opts
 * @returns {{id: string, url: string, title: string, vendor: string|null,
 *          product: string|null, addedAt: string}}
 */
function addLink(opts) {
  const o = (opts && typeof opts === 'object') ? opts : {};
  const userId = String(o.userId == null ? '' : o.userId);
  if (!_safeId(userId)) {
    throw _err('Sign in again to add a link.', 'kb.addLink: invalid userId');
  }
  const url = _cleanUrl(o.url);
  if (!url) {
    throw _err('That does not look like a web link — it must start with http:// or https://.',
      'kb.addLink: invalid url');
  }
  const vendor = _cleanMeta(o.vendor);
  const product = _cleanMeta(o.product);
  const title = (_cleanMeta(o.title) || url).slice(0, MAX_HEADING_LEN);
  ...
  if (!dir) throw _err('Could not save that link.', 'kb.addLink: bad doc dir');
  const meta = {
    id,
    url,
    title,
  ...
  try {
    ...
    try { fs.rmSync(dir, { recursive: true, force: true }); } catch { /* best effort */ }
    throw _err('Could not save that link — the data directory is not writable.',
      'kb.addLink: save failed: ' + (e && e.message));
  }
  // No _index.delete(userId): a link has no chunks and was never indexed.

  return { id, url, title, vendor, product, addedAt: meta.addedAt };
}
...
function listDocs(userId) {
  try {
    return _readMetas(String(userId == null ? '' : userId)).map(m => ({
      id: m.id,
      // A link (addLink) has no filename — falling back to the doc id here
      // would print a meaningless hex string in the table and in a GDPR
      // export, so it stays null instead.
      filename: typeof m.filename === 'string' ? m.filename : (typeof m.url === 'string' ? null : m.id),
      title: typeof m.title === 'string' && m.title ? m.title : (m.filename || m.url || m.id),
      ...
      // Present only for a link entry (addLink) — its one meaningful
      // difference from an ingested document, so the client keys off this
