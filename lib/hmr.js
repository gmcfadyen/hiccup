 ...
 *
 * The regex form (/^\(+|\)+$/g) backtracks from every start offset and is
 * quadratic on a long unbalanced run; config text is attacker-supplied.
 */
function stripOuterParens(s) {
  var str = String(s == null ? '' : s);
  var a = 0, b = str.length;
  while (a < b && str.charCodeAt(a) === 40 /* ( */) a++;
  while (b > a && str.charCodeAt(b - 1) === 41 /* ) */) b--;
  return str.slice(a, b);
}

/**
 * Parse an AudioCodes INI section header: '[Name]', '[ \Name ]'.
 *
 * Done by hand rather than by regex: the natural pattern overlaps three
 * whitespace quantifiers with a name class that itself contains a space,
 * which costs ~O(n^4) — 800 spaces after '[' took 29 seconds to reject.
 *
 * @param {string} t a trimmed line
 * @returns {{escaped: boolean, name: string}|null} null when not a section header
 */
function parseSectionHeader(t) {
  if (t.length < 3 || t.charCodeAt(0) !== 91 /* [ */ ||
      t.charCodeAt(t.length - 1) !== 93 /* ] */) return null;
  var raw = t.slice(1, -1);
  var escaped = raw.trim().charCodeAt(0) === 92 /* \ */;
  var rest = escaped ? raw.slice(raw.indexOf('\\') + 1) : raw;
  var body = rest.trim();
  // An all-blank body still matched the old pattern, because a space is in the
  // name class and the trailing \s* then trimmed it away; a tab alone did not.
  if (!body) return rest.indexOf(' ') === -1 ? null : { escaped: escaped, name: '' };
  for (var i = 0; i < body.length; i++) {
    var c = body.charCodeAt(i);
    var ok = (c >= 48 && c <= 57) || (c >= 65 && c <= 90) ||
             (c >= 97 && c <= 122) || c === 95 /* _ */ || c === 32 /* space */;
    if (!ok) return null;
  }
  return { escaped: escaped, name: body };
}
...
var VENDOR_TOKENS = {
  'oracle-acme': [
    /^[ \t]*sip-manipulation\b/mi, /^[ \t]*header-rule\b/mi, /^[ \t]*element-rule\b/mi,
    /\bin-manipulationid\b/i, /\bout-manipulationid\b/i, /^[ \t]*session-agent\b/mi,
    /^[ \t]*realm-config\b/mi, /\bmatch-val-type\b/i, /\bcomparison-type\b/i,
    /\bnew-value\b/i, /\bheader-name\b/i, /\bmsg-type\b/i, /\bpattern-rule\b/i,
    /^[ \t]*sip-interface\b/mi, /\blast-modified-by\b/i
  ],
  'audiocodes': [
    /\[\s*\\?MessageManipulations?\s*\]/i, /\bMessageManipulations?_[A-Za-z]/,
    /\bManipulationName\b/i, /\bManSetID\b/i, /\bActionSubject\b/i,
    /\bActionType\b/i, /\bRowRole\b/i, /^[ \t]*FORMAT[ \t]+\w+_Index[ \t]*=/mi,
  ...
  'ribbon': [
    ...
    /^[ \t]*criterion\b/mi, /\bcriterion\s+(messageType|header|sipMessageBody)\b/i,
...
function acConditions(condText) {
  ...
  for (var i = 0; i < parts.length; i++) {
    var seg = stripOuterParens(parts[i].text).trim();
    if (!seg) continue;
...
function parseAudioCodes(text, warnings) {
  ...
  for (var i = 0; i < all.length; i++) {
    ...
    var sec = parseSectionHeader(t);
    if (sec) { section = sec.escaped ? null : sec.name; continue; }
...
function parseRibbon(text, warnings) {
  ...
  for (var i = 0; i < all.length; i++) {
    ...
    if (pIdx === -1) {
      var pm = t.match(/^(?:signaling[ \t]*manipulation|message[ \t]*manipulation|smm)(?:[ \t]*profile)?[ \t]*(?:[:=][ \t]*)?["']?([A-Za-z0-9_.\-]+)/i);
