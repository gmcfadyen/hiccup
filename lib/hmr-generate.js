 ...
 */
const BOUNDARY = /(?=\s{1,8}\b(?:on|when|if|for|unless|only|where)\b|[,.]|$)/i;
...
function findMethods(text) {
  ...
  // 'S?' (not 's?' — t is already uppercased) so a natural plural ("outbound
  // INVITEs", "REGISTERs failing") still matches; engineers say it that way
  // far more often than the bare method name.
  return SIP_METHODS.filter((m) => new RegExp('\\b' + m + 'S?\\b').test(t));
}
