...
function dnsEvidenceFrom(ctx) {
  ...
  for (const x of ctx.aux) {
    ...
    if (!host) {
      // Bounded label length and depth (DNS allows neither to be unlimited) keep
      // the unanchored scan linear; the old pattern let `.` into the label class
      // and was quadratic on a dotted string.
      const m = /([A-Za-z0-9_-]{1,63}(?:\.[A-Za-z0-9_-]{1,63}){0,24}\.[A-Za-z]{2,63})/.exec(txt(x.summary));
...
RULES.push({
  ...
  when: (analysis, ctx) => {
    ...
    for (const m of ctx.sip) {
      ...
      if (/a?=?[ \t]{0,16}des:/.test(joined) || /\bdes:(qos|sec)/.test(joined)) {
        des++;
