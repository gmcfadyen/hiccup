...
(function () {
  ...
  function renderUsers(list) {
    ...
    list.forEach(function (u) {
      ...
      row.appendChild(el('td', u.plan === 'free' ? 'st-superuser-no' : 'st-superuser-yes',
        u.plan || 'free'));
      row.appendChild(el('td', u.isSuperuser ? 'st-superuser-yes' : 'st-superuser-no',
        u.isSuperuser ? 'yes' : 'no'));

      var tdAction = el('td');
      // Three tiers, so a select rather than a toggle -- a button that cycles
      // free -> pro -> team hides where you are about to land.
      var planSel = document.createElement('select');
      planSel.className = 'btn';
      planSel.setAttribute('aria-label', 'Plan for ' + u.email);
      ['free', 'pro', 'team'].forEach(function (p) {
        var opt = document.createElement('option');
        opt.value = p;
        opt.textContent = p;
        if ((u.plan || 'free') === p) opt.selected = true;
        planSel.appendChild(opt);
      });
      planSel.addEventListener('change', function () { setPlan(u, planSel); });
      tdAction.appendChild(planSel);
  ...
  var PLAN_EFFECT = {
    free: 'Set "{e}" back to free? If they own a team, the team itself is untouched — ' +
      'this only blocks creating or joining a NEW one.',
    pro: 'Put "{e}" on Pro? Bigger uploads and queue priority, but NOT team access.',
    team: 'Put "{e}" on Team? They will be able to create or join a team.',
  };

  async function setPlan(u, btn) {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        btn.value = was;          // nothing was saved -- do not show the new plan
        btn.disabled = false;
        return;
      }
